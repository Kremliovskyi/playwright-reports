...
- **AI Evidence Extraction (Copilot SDK):** After the CLI digest completes, the dashboard runs per-trace extraction (`src/copilot-analyzer.ts` → `analyzeRun`) over each **analyzable** failure (Before Hooks and `outcome: "skipped"` entries are excluded). `error.md` is the exclusive issue source: every fenced block under `# Error details` must produce exactly one ordered issue. The selected small model extracts issue-local facts, canonical comparison keys, interpretation/confidence/ambiguities, and exact source quotes. Code validates block cardinality/order, terminal markers, terminal-only final-state fields, and each quote against its source block. A sanitized `failure.json` projection supplies step ancestry without propagated error payloads; network errors remain supporting evidence and cannot create issues. The dashboard writes versioned **`evidence.json`** as the canonical contract, then deterministically renders **`ai-analysis.md`** from the same object without another model call. The Markdown preserves legacy terminal-summary headings and includes every issue's structured evidence. Extraction failures are written to both artifacts so downstream work can fall back to raw files.
...
- **Problem Grouping:** Once all workers finish, `src/copilot-grouper.ts` creates one tool-free session using `copilotBigModel`, high reasoning effort, and the default context tier. It embeds a flat issue dataset from the canonical evidence records; terminal-attempt fields are never copied onto earlier issues. Each issue receives a deterministic global ID and is grouped primarily by canonical failure/operation/target/difference keys, then factual step boundaries, normalized errors, network correlation, and terminal final state. The initial response must include a complete provisional grouping and may request one bounded evidence round for plausible matches with missing, ambiguous, or conflicting critical evidence. The application validates current-run IDs and section names, loads only selected error block/page/source/network snippets under strict per-request and total budgets, and sends one follow-up in the same session. It exposes no arbitrary file tool. A failed evidence round retains the provisional grouping. Final issue-ID assignments are validated exactly once; missing/unknown/duplicate IDs still receive one focused repair turn and deterministic sanitization fallback. The completion dialog reports evidence volume, repair results, request/prompt/response sizes, token/context usage, finish reason, and truncation or compaction events.
