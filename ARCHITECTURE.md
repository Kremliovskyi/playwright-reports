...
- **`config` Table:** Designed to support multiple configurations (e.g., for different teams or projects). The current implementation primarily uses an `id: 'default'` row, but the schema inherently allows scaling to multiple rows containing the physical paths to Current, Archive, and Project root directories, the serialized JSON payload for user-selected Test Runner Options, BrowserStack credentials (`browserstackUsername`, `browserstackAccessKey`, `browserstackConfig`), and Copilot settings (`copilotToken` — optional GitHub token for the SDK — and `copilotModel` — the model selected for AI failure analysis). BrowserStack and Copilot columns are added via `ALTER TABLE` with existence checks during migration.
...
## 🤖 Copilot Status Chip & Model Selection

The header hosts a **Copilot** chip (next to Run Tests) that doubles as a status indicator and the entry point for choosing the AI analysis model.

### Preflight (`GET /api/copilot-status`)

- **Mechanism:** The endpoint calls `copilotPreflight()` (`src/copilot-analyzer.ts`), which starts a `CopilotClient` (using the configured `copilotToken` when set, otherwise the host's Copilot CLI login), reads the auth status, and lists all models available to the account.
- **`ok` semantics:** `ok = authenticated && availableModels.length > 0`. The currently selected model from config is echoed back as `model` (may be `''` when nothing is selected yet) with a `modelAvailable` flag — but a stale/missing selection deliberately does **not** fail the preflight. The UI resolves the selection against `availableModels` instead.
- **Auto check on load:** The chip runs the check automatically on dashboard load and reflects the result (`checking` → `ok`/`error` with a pulse dot). The chip label shows the active model (`Copilot: <model>`).

### Model selection dialog

- **Chip click (interactive check):** On failure, the shared error dialog opens with the full preflight error (not authenticated / no models). On success, the **Select Copilot Model** modal (`#copilot-models-modal`) opens, listing every available model in a scrollable list (`.copilot-models-list`, thin custom scrollbar, header ✕ + footer Close).
- **Selection rules:** The saved `copilotModel` is highlighted with a check mark. If there is no previous selection, the **first model in the list** is pre-selected. Clicking a model persists it immediately via `POST /api/config { copilotModel }` (merge-style — the Preferences save posts only its own fields and never clobbers the model), updates the chip label, and closes the dialog.
- **Stale selection recovery:** When the check finds the saved model missing from the live list, the first available model is auto-selected and persisted. If this happens during the **automatic** load-time check (i.e. outside the dialog), a dedicated amber warning modal (`#copilot-model-warning-modal`) explains that the previous model is no longer available, the first model was selected instead, and that clicking the Copilot chip lets you pick another. During an interactive click the list itself communicates the fallback, so no extra warning is shown.
- **Analysis wiring:** `/api/failures` re-runs the preflight and resolves the model exactly like the chip: the persisted `copilotModel` when still available, otherwise the **first available model**, which is auto-persisted to the config. When a stale selection is replaced this way, the response carries an `aiWarning` that the frontend surfaces via the same amber warning modal (and refreshes the chip) — the analysis proceeds with the fallback model rather than failing. There is no env-var fallback.
...
- **AI Analysis (Copilot SDK):** After the CLI digest completes, the dashboard runs a per-trace AI analysis (`src/copilot-analyzer.ts` → `analyzeRun`) over each **analyzable** failure (Before Hooks and `outcome: "skipped"` entries are excluded). For every analyzable folder it writes an **`ai-analysis.md`** next to `error.md` — a distilled "understanding record" (step path, deepest failing step, error verbatim/normalized, network table, issues list — every error block in `error.md` explained, soft assertions included —, final page state, transient-vs-final contradiction check, root cause hypothesis, and discriminators) produced by the model selected via the header Copilot chip (persisted as `copilotModel` in the `config` table; when the selection is missing or no longer available, the first available model is auto-selected, persisted, and used, and the response carries an `aiWarning` shown by the UI). The `index.json` manifest is left untouched — the analysis lives entirely in the per-folder `ai-analysis.md` files. If the model fails for a folder, that folder's `ai-analysis.md` records a `⚠️ AI analysis failed` note plus an error block so a downstream reduce step can fall back to the raw files.
- **Concurrent analysis (3-way pool):** The per-trace analysis runs through a bounded worker pool of `ANALYSIS_CONCURRENCY = 3` (hardcoded in `copilot-analyzer.ts`) rather than a sequential loop. Each failure folder is fully isolated — it reads only its own input files (`failure.json`, `error.md`, network json), creates its **own** Copilot session, and writes only its own `ai-analysis.md` — so the three in-flight analyses never depend on or interfere with each other. A single shared `CopilotClient` hosts all sessions (the SDK is multi-session safe). The pool is I/O-bound concurrency, not OS threads: three model round-trips overlap on Node's single thread, cutting wall-clock time roughly 3× while preflight (auth + model availability) still runs once up front. A shared `cursor` hands the next folder to each worker; because `cursor++` and the `analyzed`/`failed`/`completed` counters are incremented synchronously between `await`s, no locks are needed. One folder failing (thrown error or AI-failed record) is captured per task and never aborts the others. Progress is reported via a monotonic `completed`/`total` counter (the `failure-analysis` SSE event), so the row label advances steadily even though folders finish out of order.
