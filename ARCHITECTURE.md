...
- **Selection rules:** The saved `copilotModel` is highlighted with a check mark. Clicking a model persists it immediately via `POST /api/config { copilotModel }` (merge-style — the Preferences save posts only its own fields and never clobbers the model), updates the chip label, and closes the dialog. Opening and closing the interactive picker never changes the saved model; when the saved selection is missing or stale, the list opens with no model selected.
- **Load-time stale selection recovery:** Only the **automatic page-load check** may select and persist the first available model. When it replaces a stale selection, a dedicated amber warning modal (`#copilot-model-warning-modal`) explains that the previous model is no longer available, the first model was selected instead, and that clicking the Copilot chip lets the user pick another. The interactive picker never applies this fallback.
- **Analysis wiring:** `/api/failures` treats the exact persisted `copilotModel` as mandatory. It starts one `CopilotClient`, authenticates, lists models once, and verifies the saved model before creating the digest output directory or spawning the trace-reader CLI. A missing or unavailable selection returns `409` with `code: "COPILOT_MODEL_UNAVAILABLE"`; the UI opens the model warning dialog and no digest, database analysis-run record, AI request, fallback, or config mutation occurs. The same validated client remains alive for all later per-trace sessions and is stopped in `finally` on every exit path.
