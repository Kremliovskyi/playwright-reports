...
- **Analysis wiring:** `/api/failures` runs the per-trace AI analysis with `appConfig.copilotModel`, falling back to the `COPILOT_ANALYSIS_MODEL` env default when no selection exists yet.
