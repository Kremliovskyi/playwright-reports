...
- **Problem Grouping:** Once all workers finish, `src/copilot-grouper.ts` creates one session using `copilotBigModel`, high reasoning effort, the default context tier, and no tools. The server maps the in-memory understanding records and matching manifest metadata into one compact JSON dataset embedded directly in the prompt; it does **not** send the many `ai-analysis.md` files as separate attachments. The dataset includes every valid issue with its 1-based index, test/retry/outcome metadata, step path, normalized error, network evidence, final page state, root-cause hypothesis, and discriminators. This guarantees that the initial reduce call receives the complete groupable evidence without depending on attachment injection. The model returns structured problem-to-issue references, and code deterministically renders `grouped-analysis.md` with manifest-derived retries, outcomes, folder pointers, and reconciliation. Duplicate or unknown references remain invalid. If an otherwise-valid response omits issue references, the same session receives one focused repair turn containing the previous response and the omitted issue evidence; the model returns a complete corrected grouping so each omission can join an existing problem or form a new one. The corrected response is fully revalidated. If repair fails or still omits issues, the original usable groups are preserved and remaining references are appended as `Unclassified - omitted by grouping model`; per-trace AI failures are separately appended as `Unclassified - per-trace analysis unavailable`. A malformed initial grouping response or SDK failure is still non-fatal and never removes completed per-trace output. The completion dialog shows grouping diagnostics for both outcomes, including repair results, request count, prompt/response size, elapsed time, reported token/context usage, finish reason, and truncation or compaction events.
