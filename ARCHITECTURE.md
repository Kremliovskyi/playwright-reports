...
- **Problem Grouping:** Once all workers finish, `src/copilot-grouper.ts` creates one session using `copilotBigModel` with no tools enabled. Its only attachments are `index.json` and each analyzable folder's `ai-analysis.md`. The model returns structured problem-to-issue references; code rejects missing, duplicate, or unknown references and deterministically renders `grouped-analysis.md` with manifest-derived retries, outcomes, folder pointers, and reconciliation. Per-trace failures become Unclassified. Grouping failure is non-fatal and never removes completed output.
