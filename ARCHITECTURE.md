...
- **Request deduplication:** The client tracks active report paths and ignores repeated same-page clicks. The server is authoritative: it canonicalizes the report's physical path (case-insensitive on Windows), stores it in `activeFailureAnalyses`, and returns HTTP 409 with `FAILURE_ANALYSIS_IN_PROGRESS` when that report is already active. The key is released in `finally`, including CLI, Copilot, grouping, and error exits. This lock is process-local and matches the single PM2 instance in `ecosystem.config.js`; atomic cache publication in `playwright-traces-reader` remains responsible for contention with standalone commands or other processes.
...
- **AI Evidence Extraction (Copilot SDK):** After the CLI digest completes, the dashboard runs per-trace extraction (`src/copilot-analyzer.ts` → `analyzeRun`) over each **analyzable** failure (Before Hooks and `outcome: "skipped"` entries are excluded). `error.md` is the exclusive issue source: every fenced block under `# Error details` must produce exactly one ordered issue. `src/copilot-source-evidence.ts` deterministically parses source-line IDs, error/assertion/operation/target facts, ARIA removed and added lines, terminal page labels, failure families, normalized errors, difference fingerprints, and canonical keys. Test-step titles from sanitized `failure.json` become the only allowed transition-boundary values. The source parser also matches each issue to its deepest failed trace step and exposes bounded resolution candidates from the terminal final page or completed steps occurring later in the same attempt. The selected small model returns only step/network context and constrained semantics: state labels; an issue relationship (`primary`, `downstream`, or `independent`); an optional earlier causal block for downstream issues; same-attempt resolution (`persisted`, `recovered-in-attempt`, or `unknown`); explanation; evidence-based hypothesis; confidence; and ambiguities. Failure type remains application-owned in `failureFamily`. Code validates block cardinality/order, terminal fields, issue-local labels, boundary selection, causal ordering, and exact membership of non-unknown resolution evidence in the issue's application-derived candidates. A candidate is semantically usable only when it specifically verifies the same failed condition: continuation after `expect.soft`, an unrelated passed API step, or a passing retry does not establish recovery. A pure API mismatch therefore remains `unknown` unless a later request/assertion in that attempt explicitly rechecks the contract. Invalid JSON or semantics receive one correction turn with the precise validation error and deterministic source projection. A second invalid response retains low-confidence deterministic issues with a warning instead of losing the attempt. The dashboard writes schema-v3 **`evidence.json`** as the canonical contract, then deterministically renders **`ai-analysis.md`** from the same object without a Markdown-generation model call.
...
- **Problem Grouping:** Once all workers finish, `src/copilot-grouper.ts` creates one tool-free session using `copilotBigModel`, high reasoning effort, and the default context tier. It embeds a compact grouping projection from canonical evidence and adds current-run incident hints: direct causal anchor ID, observed-state plus transition-boundary key, and content-difference fingerprint. Canonical-only duplication (ARIA source lines, block quotes, source references, repeated expected/received text, and resolution evidence already represented by outcome context) stays in `evidence.json`; long grouping prose is bounded to 1,200 characters. Grouping prioritizes incident signals before response/network signatures and treats operation, locator, target, and step path as downstream context. Materially conflicting states, boundaries, and content fingerprints remain split; resolution is descriptive impact and does not split an otherwise exact incident match. The initial response must include a complete provisional grouping and may request one bounded evidence round for plausible matches with missing, ambiguous, or conflicting critical evidence. The application validates current-run IDs and section names, loads only selected error block/page/source/network snippets under strict per-request and total budgets, and sends one follow-up in the same session. It exposes no arbitrary file tool. A failed evidence round retains the provisional grouping. Final issue-ID assignments are validated exactly once; direct causal links and exact deterministic state/content incident matches must occupy the same problem. Missing/unknown/duplicate IDs, causal splits, or exact-incident splits receive one focused repair turn; deterministic sanitization also co-locates those links if repair fails. Each high-reasoning grouping, evidence, or repair request has a 10-minute timeout. No previous-run, knowledge-base, ADO, defect, or work-item data enters grouping. The completion dialog reports evidence volume, repair results, request/prompt/response sizes, token/context usage, finish reason, and truncation or compaction events.
