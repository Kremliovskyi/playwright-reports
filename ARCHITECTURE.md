...
- **Problem Grouping:** Once all workers finish, `src/copilot-grouper.ts` creates one session using `copilotBigModel`, high reasoning effort, the default context tier, and no tools. The server maps the in-memory understanding records and matching manifest metadata into one compact JSON dataset embedded directly in the prompt; it does **not** send the many `ai-analysis.md` files as separate attachments. Each valid issue receives a deterministic global ID (`I1`, `I2`, and so on) in record/issue order, alongside test/retry/outcome metadata, step path, normalized error, network evidence, final page state, root-cause hypothesis, and discriminators. The model returns only problem-to-`issueIds` assignments; after validation, code translates those IDs back to internal folder/index references and deterministically renders `grouped-analysis.md` with manifest-derived retries, outcomes, folder pointers, and reconciliation. Missing, unknown, or globally duplicated IDs trigger one focused repair turn in the same session containing the previous complete response, the exact allowed ID set, reference diagnostics, and evidence for affected valid issues. The corrected response must reference every allowed ID exactly once. If repair fails, the initial structurally usable response is sanitized deterministically: unknown IDs and later duplicate placements are removed, empty problems are dropped, and unassigned valid IDs are appended as `Unclassified - invalid grouping references`. Per-trace AI failures are separately appended as `Unclassified - per-trace analysis unavailable`. Only a malformed initial grouping response or SDK failure prevents a grouped report; completed per-trace output is never removed. The completion dialog shows grouping diagnostics for both outcomes, including missing/unknown/duplicate repair results, request count, prompt/response size, elapsed time, reported token/context usage, finish reason, and truncation or compaction events.
