...
- **AI Evidence Extraction (Copilot SDK):** After the CLI digest completes, the dashboard runs per-trace extraction (`src/copilot-analyzer.ts` → `analyzeRun`) over each **analyzable** failure (Before Hooks and `outcome: "skipped"` entries are excluded). `error.md` is the exclusive issue source: every fenced block under `# Error details` must produce exactly one ordered issue. `src/copilot-source-evidence.ts` deterministically parses source-line IDs, error/assertion/operation/target facts, ARIA removed and added lines, terminal page labels, failure families, normalized errors, difference fingerprints, and canonical keys. Test-step titles from sanitized `failure.json` become the only allowed transition-boundary values. The selected small model returns only step/network context and constrained semantics: state labels, causal role, optional earlier causal block, explanation, evidence-based hypothesis, confidence, and ambiguities. Code validates block cardinality/order, terminal fields, issue-local labels, boundary selection, and causal ordering. Invalid JSON or semantics receive one correction turn with the precise validation error and deterministic source projection. A second invalid response retains low-confidence deterministic issues with a warning instead of losing the attempt. The dashboard writes schema-v2 **`evidence.json`** as the canonical contract, then deterministically renders **`ai-analysis.md`** from the same object without a Markdown-generation model call.
