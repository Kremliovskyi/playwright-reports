...
- **Persisted & Tracked:** Each successful digest is recorded in the database against the report's stable `uuid`, so it appears in the report's **Info** dialog under a **Digests** section. From there you can copy or open the digest directory or delete it individually. Digests are cleaned up automatically with the same logic as analysis runs: deleting or archiving the report removes its digest folders and rows, and orphaned digests (from recycled folder names) are pruned on each scan.
