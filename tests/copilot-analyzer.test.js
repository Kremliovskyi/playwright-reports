...
const modelRecord = (issues) => ({
  ...
  failingOperation: "Click submit button",
...
test("uses error.md as the exclusive issue source", async () => {
  ...
  assert.match(sentPrompt, /"failingOperation"/);
  assert.equal(result.records[0].failingOperation, "Click submit button");
});
...
test("rejects a record whose issue count differs from error.md", async () => {
  ...
  const { result } = await runAnalysis(
    modelRecord([issue, { ...issue, kind: "trace" }]),
  );
